...
export async function register(request: FastifyRequest, reply: FastifyReply) {
    ...
    try {
        ...
        if (err instanceof StudentError) {
            return reply.status(err.statusCode || 500).send({ error: {  type: err.errorType, message: err.message }})
        }

        return reply.status(500).send({ message: ERROR_MESSAGES.INTERNAL_SERVER_ERROR })
    }
