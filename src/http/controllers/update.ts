...
import { EmailAlreadyInUseError, NoEditableFieldsError, ResourceNotFoundError } from '../../use-cases/update-users'
...
export async function updateUser(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    ...
    if (err instanceof NoEditableFieldsError) {
      return reply.status(400).send({ message: 'Provide at least one editable field' })
    }
    throw err
  }
}
