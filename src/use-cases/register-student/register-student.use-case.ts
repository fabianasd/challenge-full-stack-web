...
export class RegisterStudentUseCase {
  ...
  async execute({
    ...
    cpf,
    ra,
  }: RegisterStudentInput): Promise<RegisterStudentOutput> {
    const startDate = Date.now();
    try {
      this.registerStudentGateway.addLog(
        LogType.Info,
        'Searching for student by email',
        { email },
      );
      const userWithSameEmail =
        await this.registerStudentGateway.findStudentByEmail(email);
      if (userWithSameEmail) {
        this.registerStudentGateway.addLog(
          LogType.Warn,
          'Student with email already exists',
          { email, milliseconds: Date.now() - startDate },
        );
      ...
      this.registerStudentGateway.addLog(
        LogType.Info,
        'Searching for student by document',
        { cpf, milliseconds: Date.now() - startDate },
      );
      const userWithSameCPF =
        await this.registerStudentGateway.findStudentByDocument(cpf);
      if (userWithSameCPF) {
        this.registerStudentGateway.addLog(
          LogType.Warn,
          'Student with cpf already exists',
          { cpf, milliseconds: Date.now() - startDate },
        );
        throw new StudentError(
      ...
      const student =
        await this.registerStudentGateway.registerStudent(newStudent);
